...
// CHECK-LABEL: func.func @linearize_2D_vector_load(
...
// CHECK-LABEL: func.func @linearize_2D_vector_store(
...
func.func @linearize_2D_vector_store(%arg0: memref<4x4xf16>, %arg1: vector<4x4xf16>) {
  ...
  // CHECK: vector.store %[[V0]], %[[ARG0]][%[[C1]], %[[C2]]] : memref<4x4xf16>, vector<4xf16>
  // CHECK: %[[V1:.*]] = vector.extract %[[ARG1]][1] : vector<4xf16> from vector<4x4xf16>
  // CHECK: vector.store %[[V1]], %[[ARG0]][%[[C2]], %[[C2]]] : memref<4x4xf16>, vector<4xf16>
  // CHECK: %[[V2:.*]] = vector.extract %[[ARG1]][2] : vector<4xf16> from vector<4x4xf16>
  // CHECK: vector.store %[[V2]], %[[ARG0]][%[[C3]], %[[C2]]] : memref<4x4xf16>, vector<4xf16>
  // CHECK: %[[V3:.*]] = vector.extract %[[ARG1]][3] : vector<4xf16> from vector<4x4xf16>
  // CHECK: vector.store %[[V3]], %[[ARG0]][%[[C4]], %[[C2]]] : memref<4x4xf16>, vector<4xf16>
  %c1 = arith.constant 1 : index
  %c2 = arith.constant 2 : index
...
// CHECK-LABEL: func.func @linearize_vector_load(
// CHECK-SAME:  %[[ARG:.*]]: memref<4x4x4x4xf16>) -> vector<2x2xf16> {
func.func @linearize_vector_load(%arg0: memref<4x4x4x4xf16>) -> vector<2x2xf16> {
  // CHECK: %[[C2:.*]] = arith.constant 2 : index
  // CHECK: %[[C1:.*]] = arith.constant 1 : index
